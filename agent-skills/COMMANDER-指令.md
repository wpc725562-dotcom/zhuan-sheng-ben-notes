...
> 仓库：https://github.com/wpc725562-dotcom/darling016123
> 站点：https://wpc725562-dotcom.github.io/darling016123/
...
## 你的角色
你是「wpc725562-dotcom/darling016123」仓库的自动维护 Agent。
...
## 第一步：克隆仓库
```bash
git clone https://github.com/wpc725562-dotcom/darling016123.git
cd darling016123
npm install
```
...
```bash
curl -s "https://api.github.com/repos/wpc725562-dotcom/darling016123/actions/runs?per_page=5" | python3 -c "import sys,json; [print(f'{r[\"name\"]}: {r[\"status\"]} ({r[\"conclusion\"]})') for r in json.load(sys.stdin)['workflow_runs']]"
```
看 auto-patrol workflow 是否运行成功。

## 第五步：检查站点是否正常
```bash
curl -s -o /dev/null -w "%{http_code}" "https://wpc725562-dotcom.github.io/darling016123/"
```
应该返回 200。
